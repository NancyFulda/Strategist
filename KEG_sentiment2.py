...
correct_categories, human_captions = sentiment_parsing.build_from_file(three_categories=True)
...
def gpt2_encode(text):
    # Encode some inputs
    indexed_tokens = tokenizer.encode(''.join([c for c in text if ord(c)<128]))
...
if categorization_method in ['SKIPTHOUGHT','SKIPTHOUGHT_CENTROID','SKIPTHOUGHT_NEAREST']:
    ...
    #model = 'output_context_plus_w2v_book_corpus/17_encoder.pkl'
    ...
    # ** Eigth Round **
    #model = 'output_context_book_corpus/14_encoder.pkl'
    ...
    #model = 'output_autoencoder_book_corpus_max_len_512/10_encoder.pkl'
    ...
    # ** Twelfth Round **
    #FASTTEXT_SIZE=50000
    #model = 'output_w2v_book_corpus_fasttext_50000/10_encoder.pkl'
    #model = 'output_w2v_book_corpus_fasttext_50000/20_encoder.pkl'
    #model = 'output_w2v_wikipedia_fasttext_50000/10_encoder.pkl'
    #model = 'output_w2v_wikipedia_fasttext_50000/20_encoder.pkl'
    
    FASTTEXT_SIZE=150000
    #model = 'output_w2v_book_corpus_fasttext_150000/7_encoder.pkl'
    #model = 'output_w2v_book_corpus_fasttext_150000/10_encoder.pkl'
    #model = 'output_w2v_book_corpus_fasttext_150000/20_encoder.pkl'
    #model = 'output_w2v_wikipedia_fasttext_150000/7_encoder.pkl'
    model = 'output_w2v_wikipedia_fasttext_150000/10_encoder.pkl'
    
    bse_encoder = torch.load('/mnt/pccfs/not_backed_up/nancy/docker/better_sentence_embeddings_v12/' + model)

    #dataset=dataset_fasttext.Dataset()
    dataset=dataset_fasttext.Dataset(FASTTEXT_SIZE)

elif categorization_method in ['USE_LITE_CENTROID','USE_LITE_NEAREST','USE_LARGE_CENTROID','USE_LARGE_NEAREST','BAG_OF_WORDS_CENTROID','GLOVE_BOW','INFERSENT','BAG_OF_WORDS_NEAREST','BSE_embedding','GPT2_last','GPT2_avg']:
    pass
else:
...
def categorize(text):
    if categorization_method == 'SKIPTHOUGHT':
        ...
        return val, cat_vector, distances
    elif categorization_method in ['SPACY_NEAREST', 'USE_LITE_NEAREST','USE_LARGE_NEAREST','BAG_OF_WORDS_NEAREST','GLOVE_BOW','INFERSENT','BERT1_NEAREST','BERT2_NEAREST','BSE_embedding','GPT2_last','GPT2_avg']:
...
failed_detections = [0,0,0]

baseline_false_positives = [0,0,0]
baseline_failed_detections = [0,0,0]
...
for f in tqdm(human_captions.keys()):
    ...
    if True:
        ...
        val, cat_vector, distances = categorize(''.join([c for c in text if ord(c)<128]))
        #print('val is ', val)
        ...
        human_boolean_cats=[0,0,0]
        human_baseline_categories=[0,0,0]
...
print("Num omitted: " + str(num_omitted))
print("categorization method " + categorization_method)
